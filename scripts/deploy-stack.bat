...
if  %CHANGESET_MODE% == "true"  OR %CHANGESET_MODE% == "True" (
    aws cloudformation create-change-set --stack-name %STACK_OR_CHANGESET_NAME% --change-set-name --template-body cloudformation/%TEMPLATE_NAME% --parameters file://parameters/%PARAMETERS_FILE_NAME% 
) else  (
